(function() {
  ...
  extend(SwalForm.prototype, {
    ...
    getFormValues: function(isConfirm) {
      var inputHtmlCollection = document.getElementsByClassName(
        'swal-form-field'
      );
      ...
      function toValuableAttrs(tag) {
        ...
        for (var key in attr) {
          if (attr.hasOwnProperty('email') && !email_reg.test(attr[key])) {
            var warnMsg =
              'Missing required valid email address on: ' +
              (tag.name || tag.id);
    ...
    insertFormInSwalModal: function(htmlFormString) {
      ...
      var buttonContainerTag =
        sweetAlertModal.querySelector('.sa-button-container') ||
        sweetAlertModal.querySelector('.cancel');
    ...
    var input = {
      ...
      toHtml: function() {
        ...
        if (input.type === 'textarea') {
          inputTag = t(
            "<textarea id='{id}' class='{clazz} swal-form-field' name='{name}'" +
              " value='{value}' title='{placeholder}' placeholder='{placeholder}'" +
              ' data-swal-forms-required={required}></textarea>',
            input
          );
        } else if (input.type === 'email') {
          inputTag = t(
            "<input id='{id}' type='email' class='{clazz} swal-form-field' name='{name}'" +
              " value='{value}' title='{placeholder}' placeholder='{placeholder}'" +
        ...
        function toHtmlOptions(optionsString, option) {
          return (
            optionsString + t("<option value='{value}'>{text}</option>", option)
          );
        }
      },
    };
    input.label = input.isRadioOrCheckbox()
