...
class AsyncHandler extends RedQ_Provider
{
    ...
    public function handle_ajax()
    {
      ...
      case 'save_settings':
        ...
        $reactive_all_settings = get_post_meta($ajax_data['key'], 'reactivedata', true);
        if ($reactive_all_settings == '') {
            $reactive_all_settings = [];
        }
        if (isset($ajax_data['data']['deletAll']) && count($ajax_data['data']) === 1) {
            $processed_data = [];
        } else {
            $processed_data = array_merge($reactive_all_settings, $ajax_data['data']);
        }
        if ($wpml_lang === '') {
            if (!isset($ajax_data['data']['allbox'])) {
                update_option('reactive_selected_template', '');
            }
            update_post_meta($ajax_data['key'], 'reactivedata', $processed_data);
            echo json_encode($ajax_data['data']);
        } else {
            update_post_meta($ajax_data['key'], 'reactivedata_' . $wpml_lang, $processed_data);
            echo json_encode($ajax_data['data']);
        }
        break;
    ...
    public function get_search_result($url_data, $key, $radius)
    {
        if (!empty($url_data)) {
            ...
            if (isset($url_data['datepickerrange'])) {
                $wpml_lang = 'en';
                include_once(ABSPATH . 'wp-admin/includes/plugin.php');
                if (is_plugin_active('sitepress-multilingual-cms/sitepress.php')) {
                    //plugin is activated
                ...
                $datepickerrange_search_string = $url_data['datepickerrange'][0];
                $process_dates = explode('-', $datepickerrange_search_string);
                $pickup_date = implode('-', explode('_', $process_dates[0]));
                $return_date = implode('-', explode('_', $process_dates[1]));
                $available_products_id = get_available_product_id($pickup_date, $return_date, $wpml_lang);
            }
        ...
        'datepickerrange' => isset($available_products_id) ? $available_products_id : '',
    ...
    public function process_sorting_data($builder_key, $sort_type)
    {
      ...
      case 'meta_key':
        ...
        $sorting_result = sort_by_meta_key_value($metakeys);
        break;
    }
    ...
    public function process_template_sorting($template_data)
    {
        ...
        } elseif ($template_data['saveAs'] == 'true') {
            $args = array(
