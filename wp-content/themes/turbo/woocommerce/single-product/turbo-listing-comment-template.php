...
?>
    ...
    $comment_form = array(
        'title_reply'          => have_comments() ? esc_html__('Add a Review', 'turbo') : sprintf(__('Be the first to review &ldquo;%s&rdquo;', 'turbo'), get_the_title()),
    ...
    if (get_option('woocommerce_enable_review_rating') === 'yes') {
      ...
      <label for="rating">' . esc_html__('Add a Review', 'turbo') . '</label>
      <select name="rating" id="rating">
        <option value="">' . esc_html__('Rate;', 'turbo') . '</option>
    ...
    $comment_form['comment_field'] .= '<p class="comment-form-comment">
      <label for="comment">' . esc_html__('Add a Review', 'turbo') . '</label>
...
<?php else : ?>
    <p class="woocommerce-verification-required">
        <?php esc_html_e('Only logged in customers who have purchased this product may leave a review.', 'turbo'); ?>
    </p>
<?php endif; ?>