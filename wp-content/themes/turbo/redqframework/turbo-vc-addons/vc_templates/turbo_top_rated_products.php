...
extract($atts);
$top_rated_products = Turbowp_Helper::turbowp_get_product_type('top_rated_product', 'product', $orderby, $order, $posts_per_page);
...
<div class="child-tab-wrapper <?php echo esc_attr($parent_class); ?>">
    <?php if (isset($top_rated_products) && !empty($top_rated_products)) : ?>
        <?php
        if ($transparent_img != 'active') {
            $height = '430px';
        } else {
            $height = '325px';
        }
        ...
        <ul class="nav nav-tabs" role="tablist" style="height: <?php echo esc_attr($height); ?>;">
            <?php foreach ($top_rated_products as $key => $product) : ?>
                ...
                <li role="presentation">
                    <a class="<?php echo esc_attr($class); ?>" href="#top-<?php echo esc_attr($product['ID']); ?>" role="tab" data-toggle="tab">
        ...
        <div class="tab-content">
            <?php foreach ($top_rated_products as $key => $product) : ?>
                ...
                $big_img = !empty($product['thumbnail_id']) ? (turbo_resize_image($product['thumbnail_id'], '', $width, $height, true)['url']) : '';
                ?>
                <div role="tabpanel" class="tab-pane fade <?php echo esc_attr($class); ?>" id="top-<?php echo esc_attr($product['ID']); ?>">
                        ...
                        <?php if ($transparent_img !== 'active') : ?>
                            <a href="<?php echo esc_attr($product['post_permalink']); ?>">
                                <div class="large-image-wrapper non-transparent-image">
                                    <img src="<?php echo esc_url($big_img); ?>" alt="img" />
                                </div>
                            </a>
                        <?php else : ?>
                            <?php
                            ...
                            <a href="<?php echo esc_attr($product['post_permalink']); ?>">
                                <div class="large-image-wrapper transparent-image" style="background-color: <?php echo esc_attr($product['transparent_img_bg_color']) ?>">
                                    <img src="<?php echo esc_attr($big_img); ?>" src="<?php echo esc_url($big_img); ?>" alt="img">
                                </div>
                            </a>
                        <?php endif; ?>
